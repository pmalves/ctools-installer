...
downloadCDF () {
	...
	if [[ "$BASERVER_VERSION" = "4x" ]]; then
	    ...
	    then
    	  ...
    	  URL='http://www.webdetails.pt/ficheiros/cdf/14.07.29/4.x/dist.zip'
    	fi
	else
	    if  [ $BRANCH = 'dev' ]
	    then
            ...
            URL='http://www.webdetails.pt/ficheiros/cdf/14.07.29/5.x/dist.zip'
		fi
	fi
...
downloadCDA (){
	...
	if [[ "$BASERVER_VERSION" = "4x" ]]; then
		...
		then
	    	...
	    	URL='http://www.webdetails.pt/ficheiros/cda/14.07.29/4.x/dist.zip'
	    fi
	else
		if [ $BRANCH = 'dev' ]
		then
	    	...
	    	URL='http://www.webdetails.pt/ficheiros/cda/14.07.29/5.x/dist.zip'
	    fi
...
downloadCGG (){
	...
	if [[ "$BASERVER_VERSION" = "4x" ]]; then
		...
		then
	    	...
	    	URL='http://www.webdetails.pt/ficheiros/cgg/14.07.29/4.x/dist.zip'
	    fi
	else
		if [ $BRANCH = 'dev' ]
		then
    	    ...
    	    URL='http://www.webdetails.pt/ficheiros/cgg/14.07.29/5.x/dist.zip'
    	fi
	fi
